...
const ProductDetailModal: React.FC<ProductDetailModalProps> = ({ isOpen, onClose, productId }) => {
    ...
    // Chỉ khai báo các biến dưới đây một lần
    const { data: infoProduct, isLoading, isError, refetch: refetchProductDetail } = useQuery({
        queryKey: ['product', productId], // Đảm bảo không trùng lặp với các biến khác
        queryFn: () => getInfoProductById(String(productId)),
        enabled: !!productId && isOpen, // Chỉ gọi API khi `productId` hợp lệ và modal đang mở
    ...
    return (
        <Dialog open={isOpen} handler={onClose} className="modal-overlay">
            <div className='modal-content relative z-50 mx-auto max-w-3xl h-2/3'>
                <div className='flex flex-col md:flex-row'>
                    ...
                    <div
                        onClick={onClose}
                        className='cursor-pointer  top-4 right-4 text-2xl z-10'
                    >
                        <XIcon />
                    </div>
                </div>
