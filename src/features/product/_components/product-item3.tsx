...
export default function ProductItem({ product }: Props) {
    ...
    return (
        <>
            <div className={`cursor-pointer group ${isModalOpen ? 'blur-background' : ''}`}>
                <div className='pt-6 relative pb-3 overflow-hidden'>
                    ...
                    <div className='absolute group-hover:bottom-4 transition-all group-hover:opacity-100 opacity-0 duration-500 -bottom-4 left-0 right-0 flex justify-center items-center gap-2 px-2'>
                        ...
                        <Link to={`/products/${product.productId}`} className='w-10 h-10 flex items-center justify-center border border-neutral-800 text-white bg-neutral-800 outline-none hover:opacity-90 transition-all rounded-md text-sm leading-none'>
                            <EyeIcon />
                        </Link>
                    </div>
                </div>
